...
public class AlumnoController {
	...
	public void valida() {
		...
		if (alumnoDAO.existe(nombreUsuario, pass)) {
			...
			Historial h = new Historial(null, "P", this.alumno.getId(), new Date().toInstant().toString());
			this.historialService.save(h);
			renderizaMenuAlumno();
		} else {
			menuService.log("Usuario o contraseña incorrectos", true);
		}
	...
	public void inserta() {
		...
		Alumno alumno = new Alumno(null, nombre, nombreUsu, pass, null);
		alumno = alumnoDAO.save(alumno);
		menuService.log(String.format("Añadido alumno %s  ", alumno.getNombre()), false);
